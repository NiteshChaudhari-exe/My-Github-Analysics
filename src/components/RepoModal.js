...
export default function RepoModal({ repo, onClose }) {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-opacity duration-200 ${entered && !closing ? 'opacity-100' : 'opacity-0'}`} aria-modal="true" role="dialog">
      {/* overlay */}
      <div onClick={handleClose} className={`absolute inset-0 bg-black/50 transition-opacity duration-200 ${entered && !closing ? 'opacity-100' : 'opacity-0'}`} />

      <div className={`relative w-full max-w-3xl transform transition-all duration-220 ${entered && !closing ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 -translate-y-2'}`}>
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 p-4 rounded-2xl border border-gray-700 text-white">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={handleClose} className="px-3 py-1 bg-gray-700 rounded">Close</button>
            </div>
          </div>
