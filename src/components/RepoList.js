...
export default function RepoList({ repos, onOpenRepo, isLoading, pageSize }) {
  ...
  const groups = filtered.reduce((acc, r) => { const k = r.language || 'Unknown'; (acc[k] = acc[k] || []).push(r); return acc; }, {});

  // Build the list contents separately to keep JSX balanced and easier to read
  const listContent = isLoading ? (
    ...
    Object.keys(groups).sort().map(lang => (
      <div key={lang}>
        <div className="text-xs text-gray-400 px-1 py-1 font-medium">{lang} ({groups[lang].length})</div>
        {groups[lang].map(repo => (
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-1 max-h-[50vh] overflow-auto" role="list">
        <div className="col-span-full">
          <div className="flex items-center justify-between p-1">
            <div className="text-xs text-gray-400">Showing {Math.min(filtered.length, visibleCount)} of {filtered.length}</div>
            <div className="text-xs">
              <label className="inline-flex items-center gap-2">
                <input type="checkbox" aria-label="Group by language" checked={groupByLanguage} onChange={(e) => setGroupByLanguage(e.target.checked)} />
        ...
        {listContent}
        {/* show more control */}
        {!groupByLanguage && filtered.length > visibleCount && (
          <div className="p-2 text-center">
            <button type="button" className="text-sm underline" onClick={() => setVisibleCount(filtered.length)}>Show more</button>
          </div>
        )}
      </div>
    </section>
